...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('Intelligent recruitment platform with AI-powered candidate screening')
    .setVersion('1.0')
    .addBearerAuth({
      type: 'http',
      scheme: 'bearer',
  ...
  app.getHttpAdapter().get('/health', async (req, res) => {
    ...
    try {
      const mlHealth = await fetch(`http://${process.env.ML_SERVICE_HOST || 'localhost'}:${process.env.ML_SERVICE_PORT || '8000'}/health`);
      if (mlHealth.ok) {
    ...
    try {
      const emailHealth = await fetch(`http://${process.env.EMAIL_SERVICE_HOST || 'localhost'}:${process.env.EMAIL_SERVICE_PORT || '3002'}/health`);
      if (emailHealth.ok) {
    ...
    let openaiStatus = process.env.OPENAI_API_KEY ? 'configured' : 'not_configured';
    let huggingfaceStatus = process.env.HUGGINGFACE_API_KEY ? 'configured' : 'not_configured';
